...
bool BleAcaia::isConnected() const {
    return _state == State::CONNECTED &&
           _client != nullptr && _client->isConnected();
}
...
void BleAcaia::onResult(NimBLEAdvertisedDevice* device) {
    ...
    if (prefix == "ACAIA" || prefix == "PEARL" || prefix == "LUNAR" ||
        prefix == "PYXIS" || prefix == "CINCO" || prefix == "PROCH" || prefix == "BOOKO") {
...
void BleAcaia::_subscribeAndIdentify() {
    // Acaia uses the ISSC transparent serial service
    NimBLERemoteService* svc = _client->getService(NimBLEUUID(ACAIA_SERVICE_UUID));
    if (svc) {
    ...
    static const uint8_t notifCmd[14] = {
...
void BleAcaia::_sendMessage(uint8_t msgType,
    ...
    for (size_t i = 0; i < payloadLen; i++) msg.push_back(payload[i]);
...
float BleAcaia::_decodeWeight(const uint8_t* data, size_t len) {
    if (len < 6) return -1.0f;
    if (data[0] != 0xEF || data[1] != 0xDD) return -1.0f;
